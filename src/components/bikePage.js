...
function BookBike() {
  ...
  async function handleSubmit(e) {
    ...
    if (startDate <= endDate) {
      ...
      setSpin(false);
      navigate(
        `/payment/confirm/${id}/${totalAmount}/${startDate}/${endDate}/${dates.days}`
      );
    }
  }
  return (
    <Base>
      <div className="book-div">
        <div className="container-fluid d-flex flex-wrap flex-row justify-content-center flex-column min-vh-50 align-items-center">
          <h5 className="m-3 text-uppercase fw-bold">Selected product </h5>
          <div className="bike">
            {findBike.map((prod) => (
              <div
                key={prod._id}
                className="product-bike flex-lg-row flex-column justify-content-center align-items-center"
              >
                <div>
                  <div>
                    <img src={prod.link} />
                ...
                <div className="details">
                  <table className="tab">
                    ...
                    <tr>
                      <th>{prod.price}:</th>
                      <td>
                        <span className="me-2">{prod.rate}</span>
                        {prod.priceType}
                      </td>
                    </tr>
                    <tr>
                    ...
                    <tr className="mt-2 mb-2">
                      ...
                      {prod.bookedStatus ? (
                        <td className="note fw-bold note1">booked</td>
                      ) : (
                        <td className="note fw-bold note1">Not booked</td>
                      )}
                    </tr>
                    <tr>
                      <th className="note">Availability:</th>
                      {prod.bookedStatus ? (
                        <td className="note fw-bold note1">
                          This bike wont be available until{" "}
                          <span>{prod.endDate.slice(0, 16)}</span>
                        </td>
                      ) : (
                        <td className="note fw-bold note1">Available</td>
                      )}
                    </tr>
                  </table>
                </div>
                <div className="book-btn">
                  {prod.bookedStatus ? (
                    <button
                      className="btn bg-warning text-black bk-btn"
                      disabled
                    >
                      Book now
                    </button>
                    ...
                    <form onSubmit={handleSubmit}>
                      <div>
                        ...
                        <button className="btn bg-warning text-black bk-btn">
                          Book now
                        </button>
                      </div>
                    </form>
